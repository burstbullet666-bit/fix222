...
def setup_mcp_server(hexstrike_client: HexStrikeClient) -> FastMCP:
    ...
    # Server ini sudah tanpa auth layer dan diakses lewat ngrok tunnel yang
    # domainnya berubah-ubah, jadi proteksi DNS-rebinding (yang cuma cek Host
    # header persis 127.0.0.1/localhost) dimatikan di sini.
    # transport_security diset di mcp.run() pada mcp 2.x
...
def parse_args():
    ...
    parser.add_argument("--mcp-port", type=int, default=8889,
                      help="Port to bind when using an HTTP transport (default: 8889)")
...
def main():
    ...
    try:
        ...
        if args.transport in ("sse", "streamable-http"):
            logger.info(f"🌐 MCP transport: {args.transport} on {args.mcp_host}:{args.mcp_port}")
            _ts = TransportSecuritySettings(enable_dns_rebinding_protection=False)
            mcp.run(transport=args.transport, host=args.mcp_host, port=int(args.mcp_port), transport_security=_ts)
        else:
